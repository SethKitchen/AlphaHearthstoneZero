...
def playMatches(player1, player2, EPISODES, logger, turns_until_tau0, memory = None, goes_first = 0):
    ...
    for e in range(EPISODES):
        ...
        while done == 0:
            ...
            logger.info('MCTS perceived value for %s: %f', str(state.playerTurn) ,np.round(MCTS_value,2))
            logger.info('NN perceived value for %s: %f', str(state.playerTurn) ,np.round(NN_value,2))
            logger.info('====================')
